...
//  if ((State == 11) && waittimeover) State = 12;
//  if ((State == 13) && waittimeover) State = 14;
//  if ((State == 15) && waittimeover) State = 16;
//  if ((State == 17) && waittimeover) State = 18;
//  if ((State == 19) && waittimeover) State = 20;
  
  // Jump from Intro to Clock, see states now
//  if ((State == 21) && waittimeover) State = 50;
  ...
  if ((State == 51) && ((!waittimeover && waitclockpos) || usbConnected)) State = 52;
  
  // From readposition to clock
  if ((State == 52) && ((!waittimeover && readclockpos) || usbConnected)) State = 60;
  
  // If timer is over without right position sleep again (90)
  if (((State == 51) || (State == 52)) && waittimeover) State = 90;
  
  // Jump to Power Display off
  if ((State == 61) && (currentmillis - previousclockmillis >= clocktime)) State = 90;
  ...
  switch (State) {
    ...
    case 61:
      ...
      // Show angles if enabled (SHOW_ANGLE = TRUE)
      if ((sec_changed || init_view) && SHOW_ANGLE) {
        oled.setCursor(5,6);
        oled.print("       ");
        oled.setCursor(50,6);
        oled.print("       ");
      ...
      //Show counter
      if (init_view) {
        sprintf(datebuffer, "%02u:%02u", counterWakeUp, counterShowTime);
        oled.setCursor(98,1);
        oled.print(datebuffer);
      }
