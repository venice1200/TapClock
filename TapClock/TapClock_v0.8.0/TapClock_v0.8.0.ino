...
const uint8_t xclockposmin = 15;
const uint8_t xclockposmax = 90;
const uint8_t yclockposmin = -30;
const uint8_t yclockposmax = 30;
const uint8_t xwaitposmin = -90;
const uint8_t xwaitposmax = -20;
const uint8_t ywaitposmin = -30;
const uint8_t ywaitposmax = 30;
const uint8_t xposdiff = 15;
const uint8_t yposdiff = 15;
...
const uint8_t menuValueAlarmOn = 8;     // 0=Off, 1=On
...
const uint8_t menuValueWakeupMode = 14;
...
uint8_t wakeupMode, prev_wakeupMode;
...
//    EEPROM.write(3,1);        // Watchface (unused in this version)
  ...
  //Alarm On/Off
  setupElement[menuValueAlarmOn].value = EEPROM.read(6);
  ...
  if ( setupElement[menuValueAlarmOn].value ) { 
...
// ******************* Alarm ****************************
  ...
  if (RTC.alarm(ALARM_1) && AlarmEnabled) {
    AlarmOn = true;
    setState(50);     // Enable Clock
  }
  
  // Reset Alarm with B3 if On
  ...
  if (State==61) {                                    
    if (b3delayed_pos) {
      if (setupElement[menuValueAlarmOn].value) {
        setupElement[menuValueAlarmOn].value=false;
        disableRTCInt();
      }
      else {
        setupElement[menuValueAlarmOn].value=true;
        enableRTCInt();
      }
    init_view=true;                                 // Update Display
    }
  }
  AlarmEnabled = setupElement[menuValueAlarmOn].value;
  ...
  switch (State) {
      ...
      if (!AlarmOn) {
        ...
        switch (wakeupMode) {
          case 1:
            ...
            //setState(52);        // One Position System
          break;
   ...
   break; // case 60
    ...
    case 91:
      ...
      // WakeupMode MPU
      if ( wakeupMode == 1 ) {
        attachInterrupt(digitalPinToInterrupt(MPU_INT), WakeupInterrupt, LOW);
      }
      // WakeupMode Button
      if ( wakeupMode == 2 ) {
        attachPinChangeInterrupt(digitalPinToPinChangeInterrupt(BUTTON3), WakeupInterrupt, FALLING);
      }
      ...
      if ( menuEditMode && getMenuEditMode(menuIndex) ) {
        ...
        switch(menuIndex) {
          ...
          case menuValueAlarmMode:
            ...
            if (setupElement[menuIndex].value > 5) setupElement[menuIndex].value = 1; 
          break;
    ...
    case 106:
      ...
      if ( setupElement[menuValueAlarmOn].changed ) {
        EEPROM.write(6,setupElement[menuValueAlarmOn].value);
      ...
      if ( setupElement[menuValueWakeupMode].changed ) {
        wakeupMode = setupElement[menuValueWakeupMode].value;
        EEPROM.write(4,setupElement[menuValueWakeupMode].value);
        oled.println("Set WakeupMode..."); 
      }
...
ALARM_TYPES_t getAlarmMode(byte m) {
  switch (m) {
    ...
    break;
    case 6:
    return ALM1_MATCH_HOURS;    // Alarm once each Day but manually modified by sketch.
    break;
  }
}

void enableRTCInt() {
  attachInterrupt(digitalPinToInterrupt(RTC_INT), AlarmInterrupt, FALLING);  // Attach Interrupt Routine to MCU Pin 1
